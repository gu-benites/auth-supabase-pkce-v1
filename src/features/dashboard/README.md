...
*   This page file will typically be an `async` Server Component if it needs to prefetch its own specific data not covered by the main dashboard layout.

    ```tsx
    // src/app/(dashboard)/dashboard/analytics/page.tsx
    import { AnalyticsView } from '@/features/dashboard/analytics';
    import { HydrationBoundary, QueryClient, dehydrate } from '@tanstack/react-query';
    // import { getAnalyticsDataForPage } from '@/features/dashboard/analytics/queries'; // Example query
    ...
    export default async function AnalyticsPage() {
      ...
      //     queryKey: ['analyticsPageData', /* any params */],
      //     queryFn: getAnalyticsDataForPage,
      //   });
      // } catch (error) {
      //   console.error("Failed to prefetch analytics page data:", error);
      // }
...
### Data Flow for User Profile (and other common prefetched data):
...
1.  **Server-Side Prefetching (in `src/app/(dashboard)/layout.tsx`):**
    ...
    *   It calls `await queryClient.prefetchQuery({ queryKey: ['userProfile', userId], queryFn: getCurrentUserProfile })` to fetch the detailed user profile. Other common data can be prefetched similarly.
    *   The state of this `queryClient` (containing the prefetched data) is then dehydrated and passed to the client via the `<HydrationBoundary state={dehydrate(queryClient)}>`.
...
2.  **Client-Side Hydration & Usage:**
    ...
    *   When a dashboard page initially loads on the client:
        *   TanStack Query, through the `useUserProfileQuery` hook (called by `useAuth`), encounters the `queryKey: ['userProfile', userId]`.
        *   It finds the data corresponding to this key within the `dehydratedState` provided by `HydrationBoundary`.
        *   This data is **hydrated** into the client-side TanStack Query cache.
    *   **Benefit**: The `profile` data returned by `useUserProfileQuery` (and thus available through `useAuth().profile` or `useAuth().authUser`) is populated *immediately* with the server-prefetched data. This avoids an initial client-side loading state for the profile and improves perceived performance.
    *   **After Initial Hydration**: TanStack Query takes over full client-side management of the `userProfile` data (caching, stale-time, background refetches, mutations, etc.). The `AuthSessionProvider` continues to manage the live raw Supabase session state.

3.  **Accessing via `useAuth` Hook:**
    *   Client components within the dashboard (like `UserMenu` or your feature views) use the **`useAuth` hook** (`@/features/auth/hooks/use-auth.ts`).
    *   `useAuth` internally calls `useUserProfileQuery` (which benefits from the hydrated data for its initial state) and consumes the raw session from `AuthSessionProvider`.
    *   It provides convenient access to:
        ...
        *   `authUser`: Combined `user` and `profile` object.
        *   `isAuthenticated`: Stricter boolean (session AND profile loaded).
        *   `isLoadingAuth`, `isSessionLoading`, `isProfileLoading`, and error states.

### Accessing Other Data (Feature-Specific):

1.  **Page-Level Prefetching (Optional, in feature's `page.tsx`):**
    *   If a specific dashboard page (e.g., `/dashboard/analytics`) needs unique data on initial load that isn't common to the whole dashboard, its `page.tsx` (which should be an `async` Server Component) can create its own `new QueryClient()`, prefetch that specific data, and wrap its view component in `<HydrationBoundary>`.
2.  **Client-Side Fetching (in Feature View Components):**
    ...
    *   Example:
        ...
        // import { getAnalyticsDataForPage } from './queries'; // Your server action
        import { useAuth } from '@/features/auth/hooks';

        export function AnalyticsView() {
          const { user, isAuthenticated } = useAuth(); // Gets session and profile (profile is hydrated)
          
          // const { data, isLoading, error } = useQuery({
          //   queryKey: ['analyticsPageData', user?.id],
          //   queryFn: () => getAnalyticsDataForPage(user?.id), // Server action for this page's data
...
By prefetching common data in the shared layout and potentially page-specific data in individual page components, initial load times are optimized. TanStack Query then handles client-side caching, background updates, and further data fetching. The `useAuth` hook remains the central point for accessing the user's identity and profile details throughout the dashboard features.
